 ...
 * 
 * 
 * 1.延时队列 ：  过了设置的时间段T才能从队列中取出 
 * 2.队列的泛型值V 必须继承Delay接口
 * 			CompareTo  方法接口，用来排序  
 * 			getDelay  方法接口用来获取失效的时间t
 * 
 * 
 * 
 * 
...
public class Cache<K, V> {
    ...
    public void dameonCheckOverdueKey(){
        while (true) {
            DelayedItem<K> delayedItem = queue.poll(); // poll 方法不阻塞 ，娶不到数据返回null   改用take 会减少cup的使用
            if (delayedItem != null) {
...
class DelayedItem<T> implements Delayed{
    ...
    public DelayedItem(T t,long liveTime){
        ...
        this.removeTime = TimeUnit.NANOSECONDS.convert(liveTime, TimeUnit.NANOSECONDS) + System.nanoTime();//可以取出的时间点。按纳秒计算
    }
    /**
     * 此处排序 应该改用removeTime  : 80003509 
     */
    @Override
